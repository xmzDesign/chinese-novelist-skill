...
2. **读取 `00-大纲.md`** — 找到当前章节的规划信息，提取：核心事件、承接上章、悬念钩子、出场人物、场景列表
3. 更新 `03-写作计划.json` — 将本章 `status` 设为 `"in_progress"`
...
12. 生成章节摘要 — 在 `00-大纲.md` 的章节摘要区追加（300-500字，保证连贯性参考）
13. 更新 `03-写作计划.json` — 将本章 `status` 设为 `"completed"`，填入 `wordCount`
...
8. 深度润色（去除AI味）
9. 再次字数检查
10. 在 00-大纲.md 追加 300-500 字章节摘要
