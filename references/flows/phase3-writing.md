...
3. 读取 `shuangwenConfig`：所有小说默认启用爽文专项写作和 QA
4. 检查每章是否有 `contractPath`、`qaReportPath`、`summaryPath`
5. 如果存在 `status: "in_progress"` 或 `"in_revision"` 的章节，从该章节继续
6. 如果存在 `status: "failed"`、`repairRequired: true` 且 `repairRound < maxAutoRepairRounds` 的章节，优先修复
7. 如果所有章节 `status: "pending"`，从第 1 章开始
...
6. 读取下一章规划，确认本章追读理由要能承接；结尾可以是悬念、兑现、余味、选择、反差或阶段收束
...
2. 每轮都检查章节契约、反 AI、文学质量、追读理由、结尾反套路、爽文专项、黄金三章（如适用）
