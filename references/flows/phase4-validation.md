...
Phase 4 不再把所有问题都留到最后补救。章节级字数、人物一致性、承接、场景卡、追读理由、网文顶层设计、编辑审稿、结尾反套路、爽文专项和去 AI 味应在 Phase 3 的章节 sprint 内完成。本阶段负责总验收和风险汇总。
...
3. 若 `sceneCardPolicy.enabled == true`：`sceneCardPath` 指向的场景卡存在，`sceneCardStatus == "pass"`，`sceneCardIssues` 为空
4. `qaReportPath` 指向的 QA 报告存在
5. `summaryPath` 指向的章节摘要存在
6. `wordCountPass == true`
7. `qaStatus == "pass"` 或章节已标记 `blocked`
8. `status == "completed"` 的章节必须没有阻塞项
9. `antiAiStatus == "pass"`，否则不得 completed
10. 普通章节 `literaryScore >= 80`，第1-3章 `literaryScore >= 85`
11. `readerHookStatus == "pass"`，否则不得 completed
12. 普通章节 `readerHookScore >= 80`，第1-3章 `readerHookScore >= 85`
13. `memorableMoment` 和 `chapterTurnPageHook` 非空
14. `endingStrategy` 合法，`formulaicIssues` 为空
15. 若 `webNovelDesign.enabled == true`：`immersionAnchor`、`rationalizationNote`、`coreLoopStep`、`systemRuleUse` 非空，`webNovelStatus == "pass"`，`webNovelIssues` 为空
16. `satisfactionBeats` 非空、`shuangwenStatus == "pass"`、`shuangwenIssues` 为空
17. 若 `editorGate.enabled == true`：`editorGateStatus == "pass"`，`editorGateScore` 达标，`readerLossRisks`、`editorGateIssues` 为空，`revisionLevel == "none"`
18. `reviewRoundCount >= 3`
19. `repairRequired == false`
20. `needsRecheck == false`
21. `lastFailureCodes` 为空
...
| 关系变化 | 关系转折前有行为或事件支撑 |
| 主线推进 | 连续章节没有偏离核心冲突 |
| 质量基准 | 全书持续兑现目标读者和作品核心承诺，没有偏离质量红线 |
| 场景卡 | 每章场景都有目的、冲突、信息释放、情绪兑现和局面变化 |
| 网文顶层设计 | 代入感合理化、Core Loop、主角优势来源、系统/规则逻辑和挖坑填坑账本持续成立 |
...
| 编辑审稿 | 前300字、1000字主冲突、中段、兑现和结尾均无读者流失风险 |

总验收检测规则：

1. 全书连续性、质量基准、场景卡、网文顶层设计、黄金三章、文学质量、追读力、编辑审稿、结尾反套路和爽文专项总评至少重复 3 轮
2. 每轮必须独立给出问题清单和风险等级
...
| 缺质量基准、缺场景卡 | 回到 Phase 2 或 Phase 3 补齐 |
...
| 网文顶层设计失败 | 回到 Phase 3 按 W 类失败项定向修复 |
| 场景卡失败 | 回到 Phase 3 按 SC 类失败项重做场景卡 |
| 编辑审稿失败 | 回到 Phase 3 按 E 类失败项和 revisionLevel 分层修复 |
...
- 最多执行 3 轮总验收-修复循环
- 每轮修复后必须重新执行章节 QA 的 3 轮检测和 Editor Gate，再重新执行全书总验收
...
WHILE 总验收未通过 AND validationRound < finalValidationRounds:
    ...
    FOR each failed chapter:
        ...
        回到 Phase 3 自动修复
        修复后重新执行章节 QA 至少 3 轮和 Editor Gate

    重新运行 validate_novel_project.py
    重新执行全书连续性、质量基准、场景卡、网文顶层设计、黄金三章、文学质量、追读力、编辑审稿、反套路结尾和爽文专项 3 轮总验收
```
