...
AI 自动生成大纲（含网文顶层设计字段的章节规划）、人物档案、写作计划 JSON，展示给你确认：
...
每章严格执行：读取章节任务 → 检查章节契约 → 生成场景卡 → 撰写(3000-5000字) → 去AI味润色 → post-draft hook → QA评分 → Editor Gate → 失败项定向修复 → 修复后重新三轮检测 → pre-mark-pass hook → 标记通过 → session-close hook。
