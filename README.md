...
写小说最难的是**持续写完，并且越写越不散**。这个 Skill 专为解决这个痛点而生：

- **智能问答** - 三层递进式问答，支持快速跳过和随机生成
- **网文顶层设计** - 默认设计代入感、爽点、期待感、Core Loop、合理化主角和系统/规则逻辑
- **质量基准前置** - 先明确目标读者、作品承诺、质量红线和流失雷区
- **场景卡先行** - 每章正文前先验证场景目的、冲突、信息释放和局面变化
- **编辑审稿门禁** - QA 后按真实读者流失点做商业可读性审稿
...
- **分层修复复检** - 验收失败按场景、情节、人物、文字层级定向修，修完重新三轮检测
...
为了提升小说质量，当前版本不是“写完再评价”，而是把质量拆成一条可执行的生产链路：

| 环节 | 产物 | 解决的问题 |
|---|---|---|
| 顶层设计 | `04-网文顶层设计.md` | 先确定代入感、爽点、期待感、Core Loop、金手指/系统规则和主角合理化 |
| 质量基准 | `05-质量基准.md` | 先明确目标读者、作品承诺、质量红线和读者流失雷区 |
| 章节契约 | `chapter-contracts/第XX章.md` | 把大纲转成每章必须完成的可验收任务 |
| 场景卡 | `scene-cards/第XX章.md` | 正文前确认每个场景都有目的、冲突、信息释放、情绪兑现和局面变化 |
| 正文与润色 | `第XX章-标题.md` | 按契约写正文，并去除空泛、模板化和 AI 味 |
| QA 三轮检测 | `qa/第XX章.md` | 检查字数、结构、人物、反 AI、追读力、爽文专项、网文顶层设计和黄金三章 |
| Editor Gate | `qa/第XX章.md` | 从真实读者流失点复核前300字、1000字主冲突、中段空转、兑现和结尾 |
| 分层修复复检 | `02-写作计划.json` | 按失败编号和修复层级定向修复，旧结论作废，重新检测后才能完成 |

章节只有同时通过 QA、场景卡、Editor Gate、爽文专项、反 AI、追读力、网文顶层设计和 hook 门禁，才会被标记为 `completed`。
...
用户 → ┌─────────────┐    ┌──────────────────────┐    ┌──────────────┐
       ...
       │ 逐章：契约 → 场景卡 → 初稿 → Hook → QA → Editor Gate → 修复  │
       │       → 复检 → Hook → 收口                                   │
       └──────────────────────────────────────────────────────────────┘
                            ↓
       ┌──────────────────────────────────────────────────────────────┐
...
AI 自动生成质量基准、大纲、人物档案、网文顶层设计、黄金三章设计、章节契约、场景卡占位和写作计划 JSON，展示给你确认：
...
每章严格执行：读取章节任务 → 检查章节契约 → 生成场景卡 → 撰写(3000-5000字) → 去AI味润色 → post-draft hook → QA 三轮检测 → Editor Gate → 失败项定向修复 → 修复后重新三轮检测与审稿 → pre-mark-pass hook → 标记通过 → session-close hook。
...
全稿完成后自动检查：章节文件、章节契约、场景卡、QA 报告、Editor Gate、摘要、字数、质量基准兑现、时间线、伏笔闭环、人物弧线。未闭环章节回到 Phase 3 分层修复，修复后重新发起检测，最多3轮。最终汇报前运行 stop hook。
...
└── 20260412-143000-午夜列车/
    ...
    ├── 03-黄金三章.md                  # 开篇三章设计
    ├── 04-网文顶层设计.md              # 代入感、爽点、期待感、Core Loop、系统/规则
    ├── 05-质量基准.md                  # 目标读者、核心承诺、质量红线、流失雷区
    ├── chapter-contracts/              # 每章契约
    ├── scene-cards/                    # 正文前的逐章场景设计
    ├── qa/                             # 章节 QA 与最终报告
    ├── summaries/                      # 每章摘要
...
| 法则 | 说明 |
|-----|------|
| **代入感先成立** | 用现实共鸣、规则、证据和代价合理化虚构中的不合理 |
| **爽点必须兑现** | 财富、权力、能力、身份、关系、尊严等欲望升级必须可见 |
| **期待感承上启下** | 每章同时处理填坑与挖坑，但不强制硬悬念 |
| **核心循环不断档** | 主角持续进入问题、解决冲突、获得收益、升级并面对更难挑战 |
| **合理化主角行动** | 主角优先用熟悉领域、能力来源、信息差或规则优势取胜 |
| **系统流有规则** | 金手指、系统、重生经验或职业能力必须有触发、边界、代价和升级路径 |
| **质量基准前置** | 写正文前先明确目标读者、核心承诺、质量红线和流失雷区 |
| **场景卡先行** | 每个场景都必须有目的、冲突、信息释放、情绪兑现和局面变化 |
| **编辑审稿门禁** | QA 后从读者流失点复核前300字、主冲突、中段、兑现和结尾 |
...
| **网文顶层门禁** | 启用 `webNovelDesign` 时每章必须通过 W 类代入、循环和规则检查 |
| **爽文专项** | 所有小说必须有有效爽点节拍、情绪兑现和升级可见 |
| **反套路结尾** | 连续章节不得重复同质强悬念钩子 |
| **三轮检测** | 所有检测至少 3 轮，任一轮阻塞失败不得通过 |
| **分层修复复检** | 验收失败按结构、情节、人物、文字层级修复，旧结论作废并重新检测 |
...
| `shared-infrastructure.md` | 共享机制（质量基准、场景卡、编辑审稿、偏好系统、章节契约、文学质量、追读力、三轮检测、Hook、QA、进度收口、字数脚本） |
...
| `web-novel-top-design.md` | 网文顶层设计：代入感、爽点、期待感、Core Loop、合理化主角和系统流逻辑 |
| `quality-baseline-guide.md` | 质量基准：目标读者、作品承诺、质量红线和流失雷区 |
| `scene-card-template.md` | 场景卡模板：目的、冲突、信息释放、情绪兑现和局面变化 |
| `editor-gate.md` | 编辑审稿门禁：读者流失风险和分层修复 |
| `outline-template.md` | 大纲模板（含网文顶层设计、章节规划和挖坑填坑账本） |
...
| `validate_novel_project.py` | 校验 Novel Harness 项目结构、质量基准、场景卡、Editor Gate 和章节闭环 |
| `novel_hook_guard.py` | 执行 post-draft、pre-mark-pass、stop、session-close hook，拦截未通过的新质量门禁 |
...
- 🎯 **网文顶层设计**：默认设计代入感、爽点、期待感、Core Loop 和系统/规则逻辑
- 🧱 **质量基准 + 场景卡**：正文前先锁定读者承诺和场景推进，降低水文和空转
- 🧑‍💼 **Editor Gate**：QA 后增加读者流失风险审稿和分层修复

**文档重组**
- 📁 `references/flows/`：新增 7 个流程文档（phase0-4 + shared-infrastructure）
- 📁 `references/guides/`：重组写作指南文档，新增网文顶层设计、质量基准、场景卡、编辑审稿、质量门禁、追读门禁和分层修复复检规则
