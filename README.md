...
- **Novel Harness** - 每章按契约写作、QA、修复、复检、收口，避免长篇越写越散
...
每章严格执行：读取章节任务 → 检查章节契约 → 撰写(3000-5000字) → 去AI味润色 → QA评分 → 失败项定向修复 → 修复后重新三轮检测 → 标记通过 → 写入进度快照。
...
全稿完成后自动检查：章节文件、章节契约、QA 报告、摘要、字数、时间线、伏笔闭环、人物弧线。未闭环章节回到 Phase 3 修复，修复后重新发起检测，最多3轮。
...
| `shared-infrastructure.md` | 共享机制（偏好系统、章节契约、文学质量、追读力、三轮检测、QA、进度收口、字数脚本） |
...
| `reader-hook-gate.md` | 读者钩子、幽默、亮点与追读门禁 |
