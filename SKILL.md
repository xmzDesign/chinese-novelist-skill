...
- **并行写作**（可选）：支持子Agent并行写作，通过 `03-写作计划.json` 协调状态
...
根据用户选择的写作模式（串行/并行/Teams）逐章执行创作流程。每章创作前必须读取 `00-大纲.md` 中对应章节的规划信息，严格按大纲创作。支持中断续写。 → 详见 [phase3-writing.md](references/flows/phase3-writing.md)
